    ...
    $.fn.tableExport = function (options) {
                ...
                else
                {
                    ...
                    td.each(function(i,t){
                        if($(t).find('input').length>0){
                            input = $(t).find('input').val();
                        }else if($(t).find('textarea').length>0){
                            input = $(t).find('textarea').val();
                        }else if($(t).find('select').length>0){
                            input = $(t).find('select option:selected').text();
                        }else{
                            input = '';
                        }
                        s.push(t.innerText+input);
                    });
                    arr.push(s);
                }
        ...
        switch (options.format) {
            ...
            case "xls":
                var headers = getHeaders();
                var items  = getItems();
                template = '<table border="1"><thead>%thead%</thead><tbody>%tbody%</tbody></table>';
